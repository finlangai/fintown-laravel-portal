...
function AddButton({className, children, ...props} : {className: string, children: string, props?:object}) {
    ...
    const handleNavigate = () => {
        router.visit(route('SubscriptionProgram.add'), {
            method: 'get',
        })
    }
    ...
    return (
        <Button className={`w-fit h-fit p-0  ${className}`}  {...props} onClick={handleNavigate}>
            <div style={{
                backgroundColor: stateLayerColor,
            }} 
            className="h-fit w-fit px-[16px] py-[12px]"
            >
